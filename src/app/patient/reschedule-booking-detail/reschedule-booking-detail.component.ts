...
@Component({
  selector: 'app-reschedule-booking-detail',
  templateUrl: './reschedule-booking-detail.component.html',
  styleUrls: ['./reschedule-booking-detail.component.css'],
  providers: [DoctorService, LoginService]

})
export class RescheduleBookingDetailComponent implements OnInit {
  ...
  invalidEmail: boolean = false; isloggedin: boolean = false;
  ...
  bookAppointment() {
    ...
    if (this.patientId) {
      this.doctorService.rescheduleAppointment(this.patientData).subscribe(res => {
        console.log(res);
        if (res.IsSuccess) {
